...
real*8 :: msol,mu,mp,rmin,rmax,mvir,rvir,mbcg,ahern,boltz,cmkpc,fbar,&
        fbar_temp,fc,fgas,fgas_temp,gg,guniv,pi,r500,rho0,rho0nfw,&
        rho0t,rs,ticm,x,zfeout,zfesn,zfesol, tnow, time0, time, vturb, &
        lturb, rscala, kappa, years, tend, dt, slope, alphasn, alphast, &
        snu, aml, rhojp1, rhoj

real*8, dimension(jmax) :: u(jmax),flux(jmax),ne(jmax),zfe(jmax),& 
        rhofedot(jmax),rhofe(jmax),zfest(jmax),&
        amfeiniz(jmax),amfe(jmax),gradzfe(jmax),zfeobs(jmax),&
        amfeobs(jmax),rhofeobs(jmax), zfeobsmout(jmax)
...
do j=1,jmax
   ...
   zfeobsmout(j)=zfeobs(j) - zfeout   !! subtract background metallicity
...
do j=1,jmax
   write(20,1500)rr(j)/cmkpc,zfe(j)/zfesol,zfeobs(j)/zfesol, zfeobsmout(j)/zfesol, &
...
tnow = 13.7*1e9*years
time0=tnow-5.*1e9*years
time=time0
tend=tnow
n=0

!! set diffusion coeff  (constant right now, add loop to make variable)!!

open(20,file='kappa.dat',status='unknown') !! add variable vlaues of kappa to this file !!
vturb=260.e5   !! Turbulent Velocity -- like Perseus !!
lturb=15.*cmkpc  !! Turbulence Length Scale -- this is quite uncertain !!
rscala=30.*cmkpc  !! different scaling of radius (gives smaller step size below) !!

kappa=0.11*vturb*lturb  !! constant -- if want variable, uncomment next section !!

close(20)

do while (time <= tend)
   n = n+1 !! n is not necessary right now !!
   !!!!! if (time>tend) exit !! supposedly better than a "do while" loop !!
   dt=0.4*(r(5)-r(4))**2/(2.*kappa)  !! ok for Delta_r costant !!
   time=time+dt


!! print*, 'n, dt(yr), time(Gyr) = ',n,real(dt/years),real(time/1.e9/years)
   ...
   !! density profile with source terms !! 

   do j=2,jmax-2
      rhofedot(j)=(alphast*zfest(j)/1.4+alphasn*zfesn)*rhost(j)    !! change in iron density over time !!
   enddo
   ...
   !! new outflow boundary conditions for the density *metallicity BCs are the same as before !!
   zfe(1)=zfe(2)
   zfe(jmax)=zfe(jmax-1)
   rhofe(1)=rhofe(2)
   ...
   enddo
   gradzfe(1)=0.        !! Boundary Conditions !!
   gradzfe(jmax)=0.

   do j=2,jmax-1
      rhojp1=0.5*(rho(j+1)+rho(j))  !! rho (gas) centered at "j+1" !!
      rhoj=0.5*(rho(j-1)+rho(j))    !! rho (gas) centered at "j" !!
      rhofe(j)=rhofe(j) + (dt/1.4)*(r(j+1)**2*kappa*rhojp1*gradzfe(j+1)-r(j)**2*kappa*rhoj*gradzfe(j)) &
          / (0.33333333*(r(j+1)**3-r(j)**3))    !! update the density of metals for diffusion term !!
